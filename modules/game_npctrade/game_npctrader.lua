...
-- O servidor 7.81 acrescenta os atributos da oferta depois deste marcador no
-- campo de nome, sem alterar a estrutura do pacote antigo da loja.
function controllerNpcTrader:decodeShopItemName(encodedName)
    local value = tostring(encodedName or "")
    local marker = "||OTATTR||"
    local separatorPosition = value:find(marker, 1, true)
    if not separatorPosition then
        return value, "", value
    end

    local name = value:sub(1, separatorPosition - 1)
    local details = value:sub(separatorPosition + #marker)
    local tooltip = name
    if details ~= "" then
        tooltip = tooltip .. "\n" .. details
    end
    return name, details, tooltip
end
