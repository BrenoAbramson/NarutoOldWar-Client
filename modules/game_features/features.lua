...
-- The deployed server still uses the legacy one-byte visual IDs. Flip this
-- default only when client and server are released together with uint16 IDs.
local CUSTOM_VISUAL_IDS_U16 = true

local function usesCustomVisualIdsU16()
    if not Servers_init or not G or not G.host then
        return false
    end

    local server = Servers_init[G.host]
    if not server and table.size(Servers_init) == 1 then
        local _, onlyServer = next(Servers_init)
        server = onlyServer
    end

    if server and server.visualIdsU16 ~= nil then
        return server.visualIdsU16 == true
    end

    return CUSTOM_VISUAL_IDS_U16
end
...
controller:registerEvents(g_game, {
    onClientVersionChange = function(version)
        ...
        if version >= 780 then
            ...
            -- Custom 7.81 protocol extension. Outfits already use uint16 through
            -- GameLooktypeU16; these two flags extend effects and missiles.
            -- Keep disabled until the selected server sends the same fields as uint16.
            if usesCustomVisualIdsU16() then
