...
function getItemPrice(item, single)
    ...
    return getItemPriceForAmount(item, amount)
end

function getItemPriceForAmount(item, amount)
    if getCurrentTradeType() == BUY then
...
function getMaximumBuyAmount(item)
    local maximum = getMaxAmount()

    if not ignoreCapacity:isChecked() and item.weight > 0 then
        maximum = math.min(maximum, math.floor(playerFreeCapacity / item.weight))
    end

    -- O custo da mochila nao e sempre linear, portanto testa o preco total de
    -- cada quantidade em vez de dividir o dinheiro pelo preco de uma unidade.
    while maximum > 0 and getItemPriceForAmount(item, maximum) > playerMoney do
        maximum = maximum - 1
    end
    return maximum
end
...
function refreshItem(item)
    ...
    local finalCount = 0
    if getCurrentTradeType() == BUY then
        finalCount = getMaximumBuyAmount(item)
    else
        finalCount = math.max(0, math.min(getMaxAmount(), getSellQuantityLegacy(item.ptr)))
    end

    setupPanel:enable()
    quantityScroll:setMinimum(finalCount > 0 and 1 or 0)
    quantityScroll:setMaximum(finalCount)
    quantityScroll:setValue(math.max(finalCount > 0 and 1 or 0,
        math.min(quantityScroll:getValue(), finalCount)))
    quantityScroll:setEnabled(finalCount > 1)
...
function refreshTradeItems()
    ...
    for key, item in pairs(currentTradeItems) do
        ...
        if item.attributes and item.attributes ~= '' then
            text = text .. '\n' .. item.attributes
        end
        text = text .. '\n' .. formatCurrency(item.price)
        itemBox:setText(text)
        itemBox:setTooltip(item.tooltip or item.name)

        local itemWidget = itemBox:getChildById('item')
        itemWidget:setItem(item.ptr)
        itemWidget:setTooltip(item.tooltip or item.name)
...
function onOpenNpcTrade(items)
    ...
    for key, item in pairs(items) do
        local name, attributes, tooltip = controllerNpcTrader:decodeShopItemName(item[2])
        if item[4] > 0 then
            local newItem = {}
            newItem.ptr = item[1]
            newItem.name = name
